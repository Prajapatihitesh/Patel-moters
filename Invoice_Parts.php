...
<body onload="fo()">
  ...
  <div id="title"> 
    ...
    <span>INVOICE PARTS ENTRY</span></p>  
  </div>

  <form action="Invoice_part_insert.php" method="POST" class="form">
    <?php 
      $_SESSION["e_n"]=$id;
    ?>
    ...
    <button class="but1" onclick="window.location.href='Invoice_search.php'">BACK</button>
      ...
      <?php 
          $result = mysqli_query($conn,"select* from invoce_part where invoice_no = '$id' order by SR_NO");
        ?>
...
<table border="1" style="width:100%; font-size: 25px; text-align: center;" >
  ...
  while ($res=mysqli_fetch_array($result)) {
    ...
    echo '<td>'."<a class='but1' href='Invoice_part_delete.php?no=$res[SR_NO]&id=$res[id]'> DELETE</a>".'</td>';
    echo '</tr>';
  }
  ?>
</table>  
